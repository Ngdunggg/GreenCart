...
const AddProduct = () => {
    ...
    return (
        <div className="no-scrollbar flex-1 h-[95vh] overflow-y-scroll flex flex-col justify-between items-center">
            <form onSubmit={onSubmitHandler} className="md:p-10 p-4 space-y-5 max-w-xl">
                <div>
                ...
                <div className="w-full flex flex-col gap-1">
                    ...
                    <select onChange={(e) => setCategory(e.target.value)} value={category} id="category" className="outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500/40">
                        ...
                        {categories.map((item, index) => (
                            <option key={index} value={item.text}>{item.path}</option>
                        ))}
                    </select>
                </div>
                ...
                <button onClick={onSubmitHandler} className="px-8 py-2.5 bg-primary text-white font-medium rounded">ADD</button>
            </form>
        </div>
    );
}
